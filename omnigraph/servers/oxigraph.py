...
class OxigraphConfig(ServerConfig):
    ...
    def get_docker_run_command(self, data_dir) -> str:
        ...
        """
        docker_run_command = (
            f"docker run {self.docker_options_flag}{self.docker_user_flag} -d --name {self.container_name} "
...
class Oxigraph(SparqlServer):
    ...
    def status(self) -> ServerStatus:
        ...
        if (
            logs
            and ("Listening for requests at" in logs or "Oxigraph server started" in logs)
            and self.endpoint_answers()
        ):
